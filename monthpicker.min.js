...
window.jQuery&&(window.jQuery.fn.Monthpicker=function(c,a){var b;if("undefined"===typeof c||"object"===typeof c)b="ctor";else if("string"===typeof c&&"option"===c)b="option";else{console.error("Error : Monthpicker - bad argument (1)");return}window.jQuery(this).each(function(e,d){switch(b){case "ctor":if("INPUT"!=d.tagName||"text"!=d.getAttribute("type")&&null!==d.getAttribute("type"))console.error("Monthpicker must be called on a text input");else return new Monthpicker(d,c);break;case "option":if("INPUT"!=
d.tagName||"text"!=d.getAttribute("type")&&null!==d.getAttribute("type"))console.error("Monthpicker must be called on a text input");else return e=Monthpicker.Get(d),e.UpdateOptions(a),e}})});window.Element&&function(c){c.matches=c.matches||c.matchesSelector||c.webkitMatchesSelector||c.msMatchesSelector||function(a){a=(this.parentNode||this.document).querySelectorAll(a);for(var b=-1;a[++b]&&a[b]!=this;);return!!a[b]}}(window.Element.prototype);
