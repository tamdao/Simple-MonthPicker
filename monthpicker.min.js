...
!0);this.parent.addEventListener("focusout",function(){c.instances[this.dataset.mp].Hide()},!0);this.parent.querySelector(".yearSwitch.down").addEventListener("click",function(a){c.instances[this.closest(".monthpicker").dataset.mp].PrevYear();a.preventDefault()});this.parent.querySelector(".yearSwitch.up").addEventListener("click",function(a){c.instances[this.closest(".monthpicker").dataset.mp].NextYear();a.preventDefault()});for(var a=this.parent.querySelectorAll(".monthpicker_selector>table tr:not(:first-child) td.month"),
b=0;b<a.length;b++)a[b].addEventListener("click",function(a){this.classList.contains("off")||c.instances[this.closest(".monthpicker").dataset.mp].SelectMonth(this.dataset.m);a.preventDefault()})};c.prototype.SelectMonth=function(a){var b=parseInt(a);if(isNaN(b))throw"Selected month is not a number : "+a;if(1>b||12<b)throw"Month is out of range (should be in [1:12], was "+a+")";this.selectedMonth=b;this.selectedYear=this.currentYear;this.RefreshUI();this.RefreshInputs();this.ReleaseFocus();if(null!==
